...
public:
	...
	SendFileThread(ACE_Task<ACE_MT_SYNCH>* pSFT = 0, ServerContext* pCC = 0);
	virtual ~SendFileThread(void);
	inline void SetSendThread(ACE_Task<ACE_MT_SYNCH>* pAT){m_pSendThread = pAT;}
	inline void SetClientContext(ServerContext* pCC){m_ClientContext = pCC;}
