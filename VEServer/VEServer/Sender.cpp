...
Sender::Sender (void)
	...
	act_ ("Sender ACT")
{
	m_pProactor = new ACE_Proactor(0, true);
}
	...
	u_short port)
{
	...
	if (this->wd_.open (*this, this->sock_dgram_.get_handle (), this->completion_key_, m_pProactor/*ACE_Proactor::instance ()*/) == -1)
		ACE_ERROR_RETURN ((LM_ERROR,
		"%p\n",
...
int Sender::asynch_send( ACE_INET_Addr& addr, ACE_Message_Block* pMB )
{
	m_ReceiverAddr = addr;
	return asynch_send(pMB);
}

void Sender::handle_write_dgram (const ACE_Asynch_Write_Dgram::Result &result)
{
	int iResult = result.success();
	if(iResult)
	{
		ACE_DEBUG((LM_ERROR,
			"%p\n",
